Feature: The script uses proper exit codes
  ...
  Scenario: Verify a better error message
    Given we are configured to actually run mysqldump
    When I run `db_backup_2.rb -u root big_client`
    Then the output should contain:
    """
    There was a problem running '/usr/local/mysql/bin/mysqldump -uroot big_client > big_client.sql'
    mysqldump: Got error: 1049: Unknown database 'big_client' when selecting the database
    """
    And the exit status should not be 0

  Scenario: Verify that db_backup_3 is using std err
    Given we are configured to actually run mysqldump
    When I run `db_backup_3.rb -u root big_client`
    Then the stderr should contain:
    """
    There was a problem running '/usr/local/mysql/bin/mysqldump -uroot big_client > big_client.sql'
    Got error: 1049: Unknown database 'big_client' when selecting the database
    """
    And the exit status should not be 0
